...
public class IKQueryExpressionParser {
	...
	private void addBuilder(BooleanQuery.Builder builder, Query query, Occur occur) {
		if(query != null){
			if(query instanceof BooleanQuery){
				BooleanClause[] clauses = (BooleanClause[]) ((BooleanQuery)query).clauses().toArray();
				if(clauses.length > 0
						&& clauses[0].getOccur() == occur){
	...
	private Query toBooleanQuery(Element op){
		...
		if('&' == op.type){
			addBuilder(resultBuilder, q1, Occur.MUST);
			addBuilder(resultBuilder, q2, Occur.MUST);
		}else if('|' == op.type){
			addBuilder(resultBuilder, q1, Occur.SHOULD);
            addBuilder(resultBuilder, q2, Occur.SHOULD);
		}else if('-' == op.type){
			if(q1 == null || q2 == null){
			...
			if(q1 instanceof BooleanQuery){
				...
				if(clauses.length > 0){
					...
					resultBuilder.add(q1,Occur.MUST);
				}
			...
			resultBuilder.add(q2,Occur.MUST_NOT);
		}
